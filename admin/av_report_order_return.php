...
<select class="form-control" id="orderStatus" name="orderStatus">
    ...
    <?php endforeach; ?>
                                       ...
                                       <div class="col-md-2">
    <label for="orderStatus1">Status</label>
...
<div id="orderDetails"></div>
                                        ...
                                        <table id="order2" class="table table-bordered table-hover table-striped">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width:8%">Refund date</th>
                            ...
                            WHERE final.status = '0' AND order_sts.status IN (5, 6) 
                            ...
                            ?>
                                                <tr>
                                                    ...
                                                    <?php
$cdat_query = "SELECT * FROM order_sts WHERE order_id = '$order_id' AND status = 5";  
$cdat_result = mysqli_query($con, $cdat_query);

if ($cdat_row = mysqli_fetch_array($cdat_result)) {
    $cancel_date = $cdat_row['sts_date']; 
?>
                                                    <td style="text-align: left;"><?= htmlspecialchars($cancel_date); ?>
                                                    </td>
...
?>
                                                    ...
                                                    <?php
$refdat_query = "SELECT * FROM order_sts WHERE order_id = '$order_id' AND status = 6";  
$refdat_result = mysqli_query($con, $refdat_query);

if ($refdat_row = mysqli_fetch_array($refdat_result)) {
    $refund_date = $refdat_row['sts_date']; 
?>
                                                    <td style="text-align: left;"><?= htmlspecialchars($refund_date); ?>
                                                    </td>

                                                    <?php
} else{
    echo "<td style='text-align: left;'>Not Yet</td>";
}
?>
                                                    <td style="text-align: left;">
                                                        <b> <span
...
$(document).ready(function() {
    ...
    // Initialize Select2 for the order ID dropdown
    $('#orderStatus').select2({
    ...
    // Attach event listener for the status filter
...
function filterByDateRange() {
    ...
    var selectedStatus = document.getElementById("orderStatus1").value;
    ...
    table.rows().every(function(rowIdx, tableLoop, rowLoop) {
        ...
        var statusCell = this.data()[7];  // Assuming the status is in the third column
        ...
        if (selectedStatus && !statusCell.toLowerCase().includes(selectedStatus.toLowerCase())) {
            showRow = false;
        }
        if (showRow) {
            $(this.node()).show();
