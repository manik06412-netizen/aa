...
if (isset($_POST['submit_edit'])) {
    ...
    $statement->execute(array($weight, $oprice, $sprice, $gst, $discount, $price_id));

    $_SESSION['success_message'] = 'Price updated successfully.';
    echo "<script>window.location.href='add_price.php?prd_id=" . urlencode($_GET['prd_id']) . "';</script>";
    exit;
}

if (isset($_POST['submit'])) {
    $check_cat = mysqli_query($con, "SELECT bname FROM btype WHERE bname = '" . mysqli_real_escape_string($con, $_POST['c_name']) . "' ");
    if (mysqli_num_rows($check_cat) > 0) {
        $_SESSION['error_message'] = 'Measurements already exist!';
    } else {              
        $mql = "INSERT INTO btype(bname) VALUES('" . mysqli_real_escape_string($con, $_POST['c_name']) . "')";
...
$prd_id = $_GET['prd_id'];
$qml = "SELECT * FROM dishes WHERE rs_id='" . mysqli_real_escape_string($con, $prd_id) . "'";
