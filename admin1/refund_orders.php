...
// Refund stat card counts
$refund_total_q = mysqli_query($con, "SELECT COUNT(DISTINCT f.order_id) AS cnt FROM final f INNER JOIN order_sts os ON f.order_id = os.order_id INNER JOIN (SELECT order_id, MAX(status) AS ms FROM order_sts GROUP BY order_id HAVING MAX(status) IN (5,6)) mx ON os.order_id = mx.order_id AND os.status = mx.ms WHERE f.status = '0'");
$refund_total = mysqli_fetch_assoc($refund_total_q)['cnt'] ?? 0;

$refund_amt_q = mysqli_query($con, "SELECT ROUND(SUM(f.final_amt),2) AS total FROM final f INNER JOIN order_sts os ON f.order_id = os.order_id INNER JOIN (SELECT order_id, MAX(status) AS ms FROM order_sts GROUP BY order_id HAVING MAX(status) IN (5,6)) mx ON os.order_id = mx.order_id AND os.status = mx.ms WHERE f.status = '0' GROUP BY f.order_id");
$refund_total_amt = 0;
while ($r = mysqli_fetch_assoc($refund_amt_q)) { $refund_total_amt += $r['total']; }

$refund_pending_q = mysqli_query($con, "SELECT COUNT(DISTINCT f.order_id) AS cnt FROM final f LEFT JOIN refund rf ON f.order_id = rf.order_id INNER JOIN order_sts os ON f.order_id = os.order_id INNER JOIN (SELECT order_id, MAX(status) AS ms FROM order_sts GROUP BY order_id HAVING MAX(status) IN (5,6)) mx ON os.order_id = mx.order_id AND os.status = mx.ms WHERE f.status = '0' AND rf.order_id IS NULL");
$refund_pending = mysqli_fetch_assoc($refund_pending_q)['cnt'] ?? 0;

$refund_done_q = mysqli_query($con, "SELECT COUNT(DISTINCT f.order_id) AS cnt FROM final f INNER JOIN refund rf ON f.order_id = rf.order_id INNER JOIN order_sts os ON f.order_id = os.order_id INNER JOIN (SELECT order_id, MAX(status) AS ms FROM order_sts GROUP BY order_id HAVING MAX(status) IN (5,6)) mx ON os.order_id = mx.order_id AND os.status = mx.ms WHERE f.status = '0'");
$refund_done = mysqli_fetch_assoc($refund_done_q)['cnt'] ?? 0;
?>
...
<section class="content">
    <div class="row">
        <div class="col-md-12">
            <!-- REFUND STAT CARDS -->
            <div class="av-stat-section-label"><i class="fa fa-refresh"></i>&nbsp; Refund Orders Summary</div>
            <div class="av-stat-cards-row" style="margin-bottom:20px;">
                <div class="av-stat-card" style="background:linear-gradient(135deg,#c94b4b 0%,#4b134f 100%) !important;">
                    <div class="av-stat-icon"><i class="fa fa-exclamation-circle"></i></div>
                    <div class="av-stat-info">
                        <div class="av-stat-value"><?= $refund_total; ?></div>
                        <div class="av-stat-label">Total Refund Orders</div>
                        <div class="av-stat-sub">Cancelled &amp; Refund requests</div>
                    </div>
                </div>
                <div class="av-stat-card" style="background:linear-gradient(135deg,#f7971e 0%,#ffd200 100%) !important;">
                    <div class="av-stat-icon"><i class="fa fa-inr"></i></div>
                    <div class="av-stat-info">
                        <div class="av-stat-value">&#8377;<?= number_format($refund_total_amt, 2); ?></div>
                        <div class="av-stat-label">Total Refund Amount</div>
                        <div class="av-stat-sub">Total value to be refunded</div>
                    </div>
                </div>
                <div class="av-stat-card" style="background:linear-gradient(135deg,#ee0979 0%,#ff6a00 100%) !important;">
                    <div class="av-stat-icon"><i class="fa fa-clock-o"></i></div>
                    <div class="av-stat-info">
                        <div class="av-stat-value"><?= $refund_pending; ?></div>
                        <div class="av-stat-label">Pending Refunds</div>
                        <div class="av-stat-sub">Awaiting initiation</div>
                    </div>
                </div>
                <div class="av-stat-card" style="background:linear-gradient(135deg,#11998e 0%,#38ef7d 100%) !important;">
                    ...
                    <div class="av-stat-info">
                        <div class="av-stat-value"><?= $refund_done; ?></div>
                        <div class="av-stat-label">Processed Refunds</div>
                        <div class="av-stat-sub">Refund already initiated</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="box box-info">
                <div class="box-body table-responsive">
                    ...
                    <table id="example2" class="table table-bordered table-hover table-striped">
                        ...
                        <tbody>
    <?php
    // $con is already available from header.php via inc/config.php

    // Main refund order query - status 6 = refund requested, or status 5 = cancelled (refundable)
    $query = "
    ...
    LEFT JOIN dishes ON final.product_id = dishes.rs_id
    INNER JOIN (
        SELECT order_id, MAX(status) AS max_status
        FROM order_sts
        GROUP BY order_id
        HAVING MAX(status) = 6 OR MAX(status) = 5
    ...
    // Refundable amount query
    $query_refundable = "
    SELECT final.order_id,
        ROUND(SUM(final.sel_price * final.qty - final.discount_amt), 2) AS refundable_amount
    FROM final
    WHERE final.status = '0'
    GROUP BY final.order_id";
    ...
    if (!$sql || mysqli_num_rows($sql) == 0) {
        echo '<tr><td colspan="5" class="text-center" style="padding:30px;"><i class="fa fa-inbox" style="font-size:40px;color:#ccc;"></i><br><b style="color:#aaa;">No Refund Orders Found</b></td></tr>';
    }
    ...
    while ($orders = mysqli_fetch_array($sql)) {
        ...
        $refund_amount = $refundable_amounts[$orders['order_id']] ?? 0;
        ?>
