...
function calculateTotal() {
    ...
    $('#order2 tbody tr:visible').each(function() {
        var amountText = $(this).find('td:nth-child(4)').text().trim();
    ...
    $('#totalAmount').html('<strong>' + totalFormatted + '</strong>');
    $('#totalAmountHeader').html('<strong>(' + totalFormatted + ')</strong>');
}

function calculateTotal1() {
    var total = 0;
    $('#order2 tbody tr:visible').each(function() {
        var amountText = $(this).find('td:nth-child(5)').text().trim();
        if (amountText !== '-' && amountText !== '') {
            var amount = parseFloat(amountText.replace(/[₹,]/g, ''));
            if (!isNaN(amount)) {
                total += amount;
    ...
    var totalFormatted = '₹ ' + total.toFixed(2);
    $('#totalAmount1').html('<strong>' + totalFormatted + '</strong>');
    $('#totalAmountHeader1').html('<strong>(' + totalFormatted + ')</strong>');
}

function filterByDateRange() {
    var startDateVal = document.getElementById("startDate").value;
    var endDateVal = document.getElementById("endDate").value;
    var selectedStatus = (document.getElementById("orderStatus").value || '').toLowerCase().trim();
    ...
    $('#order2 tbody tr').each(function() {
        var dateCell = $(this).find('td:nth-child(1)').text().trim();
        var statusCell = $(this).find('td:nth-child(7)').text().toLowerCase().trim();
        var parts = dateCell.split('-');
        var orderDate = (parts.length === 3) ? new Date(parts[2] + '-' + parts[1] + '-' + parts[0]) : null;
        ...
        if (selectedStatus !== '') {
            if (selectedStatus === 'placed' && !statusCell.includes('placed')) {
                showRow = false;
            } else if (selectedStatus.includes('refund') && !statusCell.includes('refund')) {
                showRow = false;
            } else if (selectedStatus === 'cancelled' && !statusCell.includes('cancelled')) {
                showRow = false;
            } else if (selectedStatus.includes('delivered') && !statusCell.includes('delivered')) {
                showRow = false;
            }
        }
