...
if (isset($_POST['shipment_submit'])) {
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        if ($existing_record['track_number'] && $existing_record['courier']) {
            $update_query = "UPDATE shipment SET origin = '$origin', destination = '$destination', custmer_name = '$cname', customer_email = '$cemail', customer_phone = '$cmobile', customer_address = '$caddress', note = '$note', status = '$shipping_status', shipment_date = '$shipment_date', shipment_time = '$shipment_time' WHERE order_id = '$orders_id'";
        } else {
            $update_query = "UPDATE shipment SET track_number = '$track_number', courier = '$courier', origin = '$origin', destination = '$destination', custmer_name = '$cname', customer_email = '$cemail', customer_phone = '$cmobile', customer_address = '$caddress', note = '$note', status = '$shipping_status', shipment_date = '$shipment_date', shipment_time = '$shipment_time' WHERE order_id = '$orders_id'";
        }
        $result_update = mysqli_query($con, $update_query);
        if ($result_update) {
            $_SESSION['flash_success'] = 'Shipping Details Updated successfully';
        } else {
            $_SESSION['flash_error'] = 'Failed to update shipping details, Please try again';
        }
    } else {
        $insert_query = "INSERT INTO shipment (user_id, order_id, track_number, courier, origin, destination, custmer_name, customer_email, customer_phone, customer_address, note, status, shipment_date, shipment_time) VALUES ('$user_id', '$orders_id', '$track_number', '$courier', '$origin', '$destination', '$cname', '$cemail', '$cmobile', '$caddress', '$note', '$shipping_status', '$shipment_date', '$shipment_time')";
        $result_insert = mysqli_query($con, $insert_query);
        if ($result_insert) {
            $_SESSION['flash_success'] = 'Shipping Details Uploaded successfully';
        } else {
    ...
    if (!empty($_POST['orders_id'])) $redirect_url .= '?order_id=' . urlencode($_POST['orders_id']);
...
<?php
$order_id = isset($_GET['order_id']) ? mysqli_real_escape_string($con, trim($_GET['order_id'])) : '';
...
if (!empty($order_id)) {
    $sql = "SELECT ad.*, f.order_id, f.user_id, f.status as final_status 
            FROM final f 
            LEFT JOIN address ad ON ad.userid = f.user_id 
            WHERE f.order_id = '$order_id' 
            LIMIT 1";
    $res = mysqli_query($con, $sql);
    if ($res && ($fetched = mysqli_fetch_array($res))) {
        $row = array_merge($row, $fetched);
    }
}
$status = $row['status'] ?? '0';
?>
// No longer needed - handler moved above
?>
<?php
// Flash toastr notifications after PRG redirect
...
document.addEventListener('DOMContentLoaded', function() {
    <?php if ($flash_ok): ?>toastr.success('<?php echo addslashes($flash_ok); ?>', 'Success');<?php endif; ?>
    <?php if ($flash_err): ?>toastr.error('<?php echo addslashes($flash_err); ?>', 'Error');<?php endif; ?>
});
</script>
<?php endif; ?>
                                ...
                                <div class="row p-t-20">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <textarea name="note" rows="5" class="form-control" placeholder="Enter shipment status note" required></textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <input type="hidden" name="cname" class="form-control" value="<?php echo htmlspecialchars($row['fname'] ?? ''); ?>" readonly required>
                                        </div>
                                        <div class="form-group">
                                            <input type="hidden" name="cemail" class="form-control" value="<?php echo htmlspecialchars($row['email'] ?? ''); ?>" readonly required>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <input type="hidden" name="cmobile" class="form-control" value="<?php echo htmlspecialchars($row['mobile'] ?? ''); ?>" readonly required>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <textarea style="display:none;" name="caddress" class="form-control" required readonly><?php echo htmlspecialchars($customer_address); ?></textarea>
                                        </div>
                                    </div>

                                    <input type="hidden" name="orders_id" class="form-control" value="<?php echo htmlspecialchars($row['order_id'] ?? ''); ?>" readonly required>
                                </div>
                            </div>
                        </div>
                    </div>
