...
<?php
    ...
    if(isset($_POST['submit'])){
        $c_name = $_POST['c_name'];
        $check = mysqli_query($con,"SELECT * FROM acc_currency where currency = '{$c_name}'");
        if(mysqli_num_rows( $check) > 0){
            echo"<script>alert('ALREADY REGISTERED');window.location.href='accepted_currency.php';</script>";
        }else{
            $insert = mysqli_query($con,"INSERT INTO acc_currency VALUES (null,'{$c_name}')");
            if($insert){
                echo"<script>alert('REGISTERED SUCCESSFULLY');window.location.href='accepted_currency.php';</script>";
            }else{
            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <h4 class="card-title">Listed of Accepted Currency</h4>
                                       ...
                                       $query=mysqli_query($con,$sql);
                                        if(!mysqli_num_rows($query) > 0 ){
                                            ...
                                            <tr>
                                                ...
                                                <td><?= $rows['currency']; ?></td>
                                                <td>
                                                    <a href="#" onclick="confirmDelete(<?=$rows['id']; ?>,'acc_currency','accepted_currency')"
                                                        class="btn btn-danger btn-flat btn-addon btn-xs m-b-10"><i
                                                            class="fa fa-trash-o" style="font-size:16px"></i> </a>

                                                    <a href="update_currency.php?cat_upd=<?=$rows['id']; ?>"
