        ...
        <div class="col-md-12">
            ...
            if (mysqli_num_rows($query) > 0) {
                while ($rows = mysqli_fetch_array($query)) {
                    ...
                    $stockLink = 'upd_stock.php?prd_id=' . htmlspecialchars($rows['rs_id']);
                  
                    $updateDescLink = 'update_desc.php?prd_id=' . htmlspecialchars($rows['rs_id']);
                    $statusChangeLink = 'products_status.php?status_id=' . htmlspecialchars($rows['d_id']);
                    $deleteConfirmLink = 'products_status.php?del_id=' . htmlspecialchars($rows['d_id']);
