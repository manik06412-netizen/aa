...
$key_id = 'rzp_test_ecMbNe517quJcM'; 
$secret = 'Wcbl8o3r0HHYBilwpRAzKbTS'; 
...
<body>
    <div id="page">
        ...
        if(isset($_POST['reg'])){
            ...
            $chek = $db->fetchOnePrepared("SELECT id FROM user WHERE (email = ? OR mobile = ?) AND user_id != ? LIMIT 1", [$email, $mob, $usrid]);
            if($chek){
                echo "<script>toastr.error('EMAIL OR MOBILE ALREADY EXISTS!','Error');</script>";
            }
            else{
                $stmt = $db->execute(
                    "UPDATE user SET fname = ?, lname = ?, email = ?, pwd = ?, mobile = ?, date = ?, status = ? WHERE user_id = ?",
                    [$fname, $lname, $email, $pwd1, $mob, $date, $sta, $usrid]
                );
                if($stmt){
                    $_SESSION['uname'] = $fname;
                    echo "<script>toastr.success('Successfully Updated! Login to continue..','Success');</script>";
                }else{
                    echo "<script>toastr.error('SORRY! Something went wrong.','Error');</script>";
                }
            }
        }
        ...
        if ($is_user_logged_in) {
            ...
            // Fallback: If no active delivery address set but user has addresses, pick first
                                     ...
                                     $fetch_details2 = mysqli_query($con, "SELECT * FROM chekout WHERE $chk_filter");
                                        ...
                                        <li class="py-2 d-flex justify-content-between">
                                            ...
                                            <span class="text-success" style="font-weight: 600;">
                                                <small class="text-muted">(-)</small> <?=$_SESSION['selectedCurrency'] ?? '₹'; ?><?= number_format($get__gst['dis_price'] ?? 0, 2); ?>
                                            </span>
                                        ...
                                        <li class="py-2 d-flex justify-content-between border-bottom pb-2">
                                            ...
                                            <span class="text-danger" style="font-weight: 600;">
                                                <small class="text-muted">(+)</small> <?=$_SESSION['selectedCurrency'] ?? '₹'; ?><?= number_format($get__gst['get_price'] ?? 0, 2); ?>
                                            </span>
                                        </li>
                                                ...
                                                if( $shiping_option==0){
                                                    ...
                                                    if ($price_total >= 500) {
                                                       ...
                                                       echo" <small class='text-muted'> (+)</small>";
                                                        echo $_SESSION['selectedCurrency'] .  
                                                        $pin_get_amount_corrent;
                                                    }
                                                }
                                        ...
                                        <?php
                                         if( $shiping_option==0){
                                            ...
                                            if ($price_total >= 499) {
                                                ...
                                                $price_total_with_ship = number_format($price_total + $pin_get_amount_corrent, 2);
                                            }
                                         }
...
?>
                                        <li>
                                            <h5>
                                                Total( disc+tax inc)<span
                                                    class="float-right"><?=$_SESSION['selectedCurrency']; ?><?= number_format($price_total_with_ship, 2); ?>
                                                    </span>
                                            </h5>
                                        </li>
                                        ...
                                        <div class="text-center">
                                            <input type="hidden" class="promo_discount1" id="promo_price" name="promo_amt" value="0">
                                            <span id="promo_error" class="text-danger h4"></span>
                                        </div>

                                        <li class="py-3 border-top border-bottom my-2" style="list-style: none;">
                                            <div id="newsletter" class="news_coupon" style="display: flex; gap: 8px; width: 100%;">
                                                <input type="text" name="subs" id="email_newsletter"
                                                    class="form-control" placeholder="Enter Coupon Code" 
                                                    style="border-radius: 6px; text-transform: uppercase; font-weight: 600; letter-spacing: 0.5px;"
                                                    onkeypress="if(event.keyCode===13){event.preventDefault(); Find_coupon();}">
                                                <button type="button" onclick="Find_coupon()" class="btn btn-success" id="submit-newsletter" style="border-radius: 6px; font-weight: 700; white-space: nowrap; padding: 6px 18px;">
                                                    <i class="fa fa-tag"></i> Apply
                                                </button>
                                            </div>
                                            <div class="mt-2" id="coupon_applied_badge" style="display:none; justify-content: space-between; align-items: center; background: #ecfdf5; border: 1px solid #10b981; padding: 8px 12px; border-radius: 6px;">
                                                <span style="color: #065f46; font-size: 13px; font-weight: 600;" id="coupon_badge_text"></span>
                                                <button type="button" onclick="Remove_coupon()" class="btn btn-outline-danger btn-xs" style="padding: 2px 8px; font-size: 11px;">Remove</button>
                                            </div>
                                            <div class="text-left mt-1"><span id="discount-message" style="font-size: 13px;"></span></div>
                                        </li>
                                        ...
                                        <li class="text-dark">
                                            <h4 class="text-success">
                                                ...
                                                <span class="float-right total-cost">
                                                    ...
                                                    <span id="displayed-total"><?= number_format((float)str_replace(',', '', (string)$price_total_with_ship), 2); ?></span>
                                                </span>
                                            </h4>
                                        </li>
                                    ...
                                    <input type="hidden" id="address_id_check" class="add_input" name="address_id"
                                        value="<?php echo  $cor_id; ?>" required>
                                    ...
                                    <div class="form-group">
                                        <label class="container_check">I accept the
                                            <a href="#">Terms and conditions</a>.
                                            <input type="checkbox" id="termsCheckbox" required>
                                            <span class="checkmark"></span>
                                        </label>
                                        <div id="error-message" style="color: red; display: none;">You must accept the
                                            terms and conditions.</div>
                                    </div>

                                    <?php
                                    $order_id = 'order_' . time() . rand(100, 999);
                                    $ans = max(100, intval(floatval($price_total_with_ship ?? $price_total) * 100)); // Converting to paise
                                    try {
                                        if (isset($api) && !empty($ans)) {
                                            $order = $api->order->create(array(
                                                'receipt' => !empty($reference) ? $reference : ('rec_' . time()),
                                                'amount' => $ans, 
                                                'currency' => 'INR',
                                    ...
                                    } catch (\Throwable $rzp_err) {
                                        // Fallback generated order_id
                                    }
                                    ?>
    ...
    <script>
    const originalFinalAmount = parseFloat(String("<?=$price_total_with_ship ?>").replace(/,/g, '')) || 0;

    function Find_coupon() {
        let discount_message = document.getElementById("discount-message");
        let email_newsletter_el = document.getElementById('email_newsletter');
        let code = (email_newsletter_el ? email_newsletter_el.value : '').trim();
        ...
        if (code.length === 0) {
            ...
            if (discount_message) {
                discount_message.innerHTML = "<span style='color:red;'>Please enter a coupon code</span>";
            }
            return;
        }

        let applyBtn = document.getElementById('submit-newsletter');
        if (applyBtn) {
        ...
        xhr.onload = function() {
            if (applyBtn) {
                ...
                applyBtn.innerHTML = "<i class='fa fa-tag'></i> Apply";
            }

            try {
                let response = JSON.parse(this.responseText);
                if (response.success) {
                    let discount_per = parseFloat(response.discount) || 0;
                    let discount_amt = (originalFinalAmount * discount_per) / 100;
                    let discounted_price = Math.max(0, originalFinalAmount - discount_amt);

                    document.getElementById("displayed-total").textContent = discounted_price.toFixed(2);
                    document.getElementById('promo_price').value = discount_amt.toFixed(2);
                    document.getElementById("final_price_show").value = discounted_price.toFixed(2);

                    // Show applied badge
                    let badge = document.getElementById("coupon_applied_badge");
                    let badgeText = document.getElementById("coupon_badge_text");
                    let inputDiv = document.getElementById("newsletter");

                    if (badge && badgeText) {
                        badgeText.innerHTML = "<i class='fa fa-check-circle'></i> <strong>" + response.code + "</strong> (" + discount_per + "% OFF) - Saved ₹" + discount_amt.toFixed(2);
                        badge.style.display = "flex";
                    }
                    if (inputDiv) inputDiv.style.display = "none";
                    if (discount_message) discount_message.innerHTML = "";

                    if (typeof toastr !== 'undefined') {
                        toastr.success("Coupon applied! You saved ₹" + discount_amt.toFixed(2), "Coupon Applied");
                    }
                } else {
                    if (discount_message) {
                        discount_message.innerHTML = "<span style='color:#dc2626; font-weight:600;'><i class='fa fa-times-circle'></i> " + (response.message || "Invalid coupon code") + "</span>";
                    }
                    if (typeof toastr !== 'undefined') {
                        toastr.error(response.message || "Invalid coupon code", "Coupon Error");
                    }
                }
            } catch(e) {
                console.error("Coupon parse error:", e, this.responseText);
                if (discount_message) {
                    discount_message.innerHTML = "<span style='color:red;'>Error validating coupon code</span>";
                }
            }
        };
        xhr.onerror = function() {
            ...
            if (discount_message) {
                discount_message.innerHTML = "<span style='color:red;'>Network error while applying coupon</span>";
            }
        };
        xhr.send(data);
    }
    ...
    function Remove_coupon() {
        document.getElementById("displayed-total").textContent = originalFinalAmount.toFixed(2);
        document.getElementById('promo_price').value = "0";
        document.getElementById("final_price_show").value = originalFinalAmount.toFixed(2);
        ...
        if (email_newsletter_el) email_newsletter_el.value = "";
        if (discount_message) discount_message.innerHTML = "<span style='color:#6b7280;'>Coupon removed</span>";

        if (typeof toastr !== 'undefined') {
            toastr.info("Coupon removed", "Notice");
        }
    }
    </script>
                ...
                <form id="pinForm" action="checkout_address.php" method="POST">
                    <div class="contact-form-container ">
                        <div class="row ">
                            ...
                            <div class="col-lg-6">
                                <span id="pinError" style="color: #800000;margin-bottom:5px;"></span>
                                <p class="form-row">
                                    <input type="number" name="pin" min="000000" max="999999" id="pin"
                                        placeholder="Pin code" class="form-control" required>
                                </p>
                            </div>
